...
loong64 uses the portable scalar fallback: Go's loongarch64 assembler currently
exposes no vector->GPR move (the `vpickve2gr` family), so LSX cannot read a lane
out to a register to locate the first difference. Pending an upstream `cmd/asm`
addition.
